...
def prepare_create(request):
    ...
    if not envs.env.get('DB_CREATED'):
        create_database(y, env=envs.env)
        envs.env['DB_CREATED'] = True
    if x and y: create_stack(x,y, env=envs)
...
def query_args(func):
    def wrapper(*args, **kwargs):
        ...
        if request.method == 'GET':
            ...
            if args_r.get('site-url'): envs.env['URL'] = args_r.get('site-url')
...
def list_stack(q_args=None, **kwargs):
    if not envs.env.get('URL'): return render_template('get_url.html')
    r = RancherAPI(key=envs.env.get('RANCHER_API_KEY'), secret=envs.env.get('RANCHER_API_SECRET'), base_url=envs.env.get('URL'))
    r.set_project()
    stacks =[(i.get('id'), i.get('name'), envs.env['URL']) for i in r.get_stack_list().get('data') if i.get('group') == "io.rancher.service.create_by_app"]
    return render_template('stack_list.html', stacks=stacks, base_url=envs.env.get('URL'))
